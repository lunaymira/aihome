// Дом · Счётчики — версия v5 с итогами по периодам
// Показывает расход между датами и сумму за период
...
const STORAGE_KEY = 'aihome_meters_readings_v5';
const PRICES_KEY = 'aihome_meters_prices_v5';
...
function renderHistory() {
    ...
    historyList.innerHTML = sorted.map(r => {
        ...
        let periodText = '';
        if (!isFirst) {
            const prevElec = getPreviousByDate(r.electricityDate, 'electricity');
            const prevWater = getPreviousByDate(r.waterDate, 'water');
            
            const fromElec = prevElec ? formatShortDate(prevElec.electricityDate) : '—';
            const fromWater = prevWater ? formatShortDate(prevWater.waterDate) : '—';
            
            periodText = `Период: ⚡ ${fromElec} → ${formatShortDate(r.electricityDate)} | 💧 ${fromWater} → ${formatShortDate(r.waterDate)}`;
        }
            ...
            <div class="history-item" data-id="${r.id}">
                <div class="history-item-header">
                    <div style="display:flex;flex-direction:column;gap:0.25rem;">
                        <span class="history-date">${formatShortDate(r.electricityDate)} / ${formatShortDate(r.waterDate)}</span>
                        ${!isFirst ? `<span style="font-size:0.75rem;color:var(--muted);">${periodText}</span>` : ''}
                    </div>
                    <div style="display:flex;flex-direction:column;align-items:flex-end;gap:0.25rem;">
                        <span class="history-total">${isFirst ? 'База' : formatCurrency(r.totalCost)}</span>
                        ${!isFirst ? `<span style="font-size:0.75rem;color:var(--muted);">за период</span>` : ''}
                    </div>
                </div>
                <div class="history-details">
                    <div class="history-detail">
                        ...
                        <span class="history-detail-value">
                            ${r.hasPreviousElectricity 
                                ? `${r.electricityTotalConsumption.toFixed(1)} кВт·ч`
                                : 'база'}
                        </span>
                        ${r.hasPreviousElectricity 
                            ? `<span class="history-detail-cost">${formatCurrency(r.electricityTotalCost)}</span>`
                            : ''}
                    </div>
                    <div class="history-detail">
                        ...
                        <span class="history-detail-value">
                            ${r.hasPreviousWater 
                                ? `${r.coldWaterConsumption.toFixed(3)} м³`
                                : 'база'}
                        </span>
                        ${r.hasPreviousWater 
                            ? `<span class="history-detail-cost">${formatCurrency(r.coldWaterCost)}</span>`
                            : ''}
                    </div>
                    <div class="history-detail">
                        ...
                        <span class="history-detail-value">
                            ${r.hasPreviousWater 
                                ? `${r.hotWaterConsumption.toFixed(3)} м³`
                                : 'база'}
                        </span>
                        ${r.hasPreviousWater 
                            ? `<span class="history-detail-cost">${formatCurrency(r.hotWaterCost)}</span>`
                            : ''}
                    </div>
                </div>
            </div>
...
function showDetails(r) {
    ...
    msg += `═══ ЭЛЕКТРИЧЕСТВО ═══\n`;
    msg += `Дата: ${formatLongDate(r.electricityDate)}\n`;
    
    if (r.hasPreviousElectricity && prevElec) {
        msg += `Период: ${formatShortDate(prevElec.electricityDate)} → ${formatShortDate(r.electricityDate)}\n\n`;
        msg += `Т1 (день):\n`;
        msg += `  ${prevElec.electricityT1Reading.toFixed(2)} → ${r.electricityT1Reading.toFixed(2)} = ${r.electricityT1Consumption.toFixed(2)} кВт·ч\n`;
        msg += `  ${r.electricityT1Consumption.toFixed(2)} × ${r.electricityT1Price.toFixed(2)} ₽ = ${formatCurrency(r.electricityT1Cost)}\n\n`;
        
        msg += `Т2 (ночь):\n`;
        msg += `  ${prevElec.electricityT2Reading.toFixed(2)} → ${r.electricityT2Reading.toFixed(2)} = ${r.electricityT2Consumption.toFixed(2)} кВт·ч\n`;
        msg += `  ${r.electricityT2Consumption.toFixed(2)} × ${r.electricityT2Price.toFixed(2)} ₽ = ${formatCurrency(r.electricityT2Cost)}\n\n`;
        
        msg += `⚡ Итого электричество: ${formatCurrency(r.electricityTotalCost)}\n`;
    } else {
        msg += `Т1: ${r.electricityT1Reading.toFixed(2)} кВт·ч (база)\n`;
        msg += `Т2: ${r.electricityT2Reading.toFixed(2)} кВт·ч (база)\n`;
    }
    
    msg += `\n═══ ВОДА ═══\n`;
    msg += `Дата: ${formatLongDate(r.waterDate)}\n`;
    
    if (r.hasPreviousWater && prevWater) {
        msg += `Период: ${formatShortDate(prevWater.waterDate)} → ${formatShortDate(r.waterDate)}\n\n`;
        
        msg += `Холодная вода:\n`;
        msg += `  ${prevWater.coldWaterReading.toFixed(3)} → ${r.coldWaterReading.toFixed(3)} = ${r.coldWaterConsumption.toFixed(3)} м³\n`;
        msg += `  ${r.coldWaterConsumption.toFixed(3)} × ${r.coldWaterPrice.toFixed(2)} ₽ = ${formatCurrency(r.coldWaterCost)}\n\n`;
        
        msg += `Горячая вода:\n`;
        msg += `  ${prevWater.hotWaterReading.toFixed(3)} → ${r.hotWaterReading.toFixed(3)} = ${r.hotWaterConsumption.toFixed(3)} м³\n`;
        msg += `  ${r.hotWaterConsumption.toFixed(3)} × ${r.hotWaterPrice.toFixed(2)} ₽ = ${formatCurrency(r.hotWaterCost)}\n\n`;
        
        msg += `💧 Итого вода: ${formatCurrency(r.waterTotalCost)}\n`;
    } else {
        msg += `Холодная: ${r.coldWaterReading.toFixed(3)} м³ (база)\n`;
        msg += `Горячая: ${r.hotWaterReading.toFixed(3)} м³ (база)\n`;
    }
    
    if (r.hasPreviousElectricity || r.hasPreviousWater) {
        msg += `\n═══════════════\n`;
        msg += `💰 ИТОГО ЗА ПЕРИОД: ${formatCurrency(r.totalCost)}`;
    } else {
        msg += `\n(Базовое показание — расчёт начнётся со следующего)`;
    }
